  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <ScatterChart margin={{ top: 10, right: 18, bottom: 24, left: 8 }}>
        ...
        <Tooltip content={<Tip />} />
        <Legend />
        <Scatter name="Carico" data={result.chartCollaudo || []} line={{ stroke: T.cycle2, strokeWidth: 2 }} fill={T.cycle2} />
...
export function Results({ result, data, setData, chartRef }) {
  return (
    <div>
      <div className="cards">
        <ResultCard label="Gradini compilati" value={result.measuredCount} unit={`/${result.rows?.length || 0}`} color={T.accentBlue} sub="letture inserite" />
        <ResultCard label="Cedimento a 100%" value={fmt(result.maxDisplacement, 3)} unit="mm" color={T.cycle1} sub="cedimento medio" />
      ...
      <div className="table-box">
        <div className="table-title">Tabella carico massimo, pressioni e letture</div>
      ...
      <div className="note"><b>Nota tecnica:</b> l’app usa il carico massimo di prova inserito dal tecnico. Gli step kN e i bar sono automatici con martinetto fisso 30 ton (= 294,30 kN) e manometro fisso 700 bar. Le letture dei 3 comparatori restano manuali e vengono riportate come inserite.</div>
    </div>
  );
}
