...
export function exportCsv({ data, result }) {
  const rows = [
    ...
    ["Ne kN", data.designLoadSLE, "Nc kN", data.testLoad, "Lunghezza m", data.length, "Martinetto fisso", "30 ton", "Rif. kN", fmt(result.pressureReferenceLoadKn, 2), "Manometro", "700 bar"],
    [],
    ["N", "Ciclo", "Percentuale", "Pressione bar", "Carico calcolato kN", "Rif. martinetto kN", "Cedimento medio mm", "Comparatore 1", "Comparatore 2", "Comparatore 3"],
    ...result.rows.map((r) => [r.stepNo, r.cycleLabel, r.label, fmt(r.pressure, 2), fmt(r.load, 2), fmt(result.pressureReferenceLoadKn, 2), fmt(r.reading, 3), fmt(r.readings?.c1, 3), fmt(r.readings?.c2, 3), fmt(r.readings?.c3, 3)]),
    [],
    ["Esito", data.outcome],
